...
package net.mcreator.analcraft.world.dimension;
...
import net.mcreator.analcraft.procedures.DungeonVanPlayerLeavesDimensionProcedure;
...
public class DungeonVanDimension extends AnalCraftModElements.ModElement {
	@ObjectHolder("anal_craft:dungeon_van")
	public static final ModDimension dimension = null;
	@ObjectHolder("anal_craft:dungeon_van_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("dungeon_van"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("anal_craft:dungeon_van")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("anal_craft:dungeon_van"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("anal_craft:dungeon_van"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new DungeonVanItem().setRegistryName("dungeon_van"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("dungeon_van_portal");
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("anal_craft:aaaaaaa"))),
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = DungeonVanDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == VanportalblockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == VanportalblockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == VanportalblockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("dungeon_van_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "dungeon_van_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = DungeonVanDimension.CustomPortalBlock.createPatternHelper(this.world,
						blockpos);
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? VanportalblockBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, VanportalblockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getFrom() == type) {
			{
				...
				DungeonVanPlayerLeavesDimensionProcedure.executeProcedure($_dependencies);
			}
		}
		if (event.getTo() == type) {
			{
				...
				$_dependencies.put("z", z);
				$_dependencies.put("world", world);
				DungeonVanPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
