...
import net.mcreator.analcraft.entity.VanEntity;
import net.mcreator.analcraft.entity.SlaveaEntity;
import net.mcreator.analcraft.block.DirtVanBlock;
import net.mcreator.analcraft.block.DerevovanBlock;
...
public class VanDungeonBiome extends AnalCraftModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0f).depth(0.1f).scale(0.04f).temperature(0.5f).precipitation(Biome.RainType.NONE)
					.category(Biome.Category.DESERT).waterColor(-1).waterFogColor(-1).surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(
							DirtVanBlock.block.getDefaultState(), BlackstoneBlock.block.getDefaultState(), BlackstoneBlock.block.getDefaultState())));
			setRegistryName("van_dungeon");
			DefaultBiomeFeatures.addCarvers(this);
			this.addStructure(Feature.STRONGHOLD.withConfiguration(IFeatureConfig.NO_FEATURE_CONFIG));
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(DerevovanBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(Blocks.CAVE_AIR.getDefaultState()))).baseHeight(14)
									.setSapling((net.minecraftforge.common.IPlantable) Blocks.JUNGLE_SAPLING).build())
					.withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(1, 0.1F, 1))));
			this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(VanEntity.entity, 25, 4, 16));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(SlaveaEntity.entity, 15, 4, 4));
		}
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DirtVanBlock.block.getDefaultState().getBlock() || ground == BlackstoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == DirtVanBlock.block.getDefaultState().getBlock()
									|| ground2 == BlackstoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), BlackstoneBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, DerevovanBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == DerevovanBlock.block.getDefaultState().getBlock()
					|| blockType == Blocks.CAVE_AIR.getDefaultState().getBlock() || blockType == DirtVanBlock.block.getDefaultState().getBlock()
					|| blockType == BlackstoneBlock.block.getDefaultState().getBlock();
		}
